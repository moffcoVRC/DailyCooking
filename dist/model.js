...
export function initialState(){return {version:1,plannerVersion:4,plans:{},checks:{},history:{},family:['自分','娘','孫'].map((name,i)=>({id:String(i),name,ng:[],dislike:[]}))};}
...
export function migrateState(s){if(s.plannerVersion!==4){s.plans={};s.checks={};delete s.excluded;s.plannerVersion=4;}return s;}
...
export function mainIngredient(menu){
  ...
  const tags=menu.tags,ingredients=menu.ingredients.map(i=>i.name).join(' ');
  for(const [tag,group,pattern] of [['牛肉','beef',/牛(?:肉|こま|薄切り|ひき|もも|バラ)/],['豚肉','pork',/豚|ハム|ベーコン|ウインナー|ソーセージ/],['鶏肉','chicken',/鶏(?:肉|もも|むね|胸|ひき|ささみ)|ささみ/],['魚','fish',/鮭|サケ|サバ|さば|たら|タラ|ぶり|ブリ|ツナ|カニカマ|しらす/]])if(tags.includes(tag)||pattern.test(ingredients))return group;
  if(menu.ingredients.some(i=>/豆腐|厚揚げ|油揚げ/.test(i.name)))return 'tofu';
  if(tags.includes('卵'))return 'egg';
  return 'vegetable';
}
export function ingredientLabel(menu){return {beef:'牛肉',pork:'豚肉',chicken:'鶏肉',fish:'魚',tofu:'豆腐・厚揚げ',egg:'卵',vegetable:'野菜'}[mainIngredient(menu)];}
...
export function ensurePlan(s,date,menus){
  ...
  // A single remaining category cannot fill consecutive days safely.
  if(cycle.length===1)cycle.push(null);
  const used=new Set();
