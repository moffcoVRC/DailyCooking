...
function worker(fail=false){
  ...
  const context={Request:class extends Request {constructor(url,options){super(new URL(url,'https://example.test/DailyCooking/'),options);}},Response,URL,
    ...
    caches:{open:async()=>({addAll:async requests=>{if(fail)throw Error('offline');requests.forEach(r=>assert.equal(r.cache,'reload'));calls.push('cached');}}),keys:async()=>['mainichi-dinner-v5','mainichi-dinner-v6','other-app'],delete:async name=>calls.push(name),match:async()=>new Response('offline copy')},
...
test('PWA activates only after cache is complete and removes only its obsolete cache',async()=>{const {events,calls}=worker();await dispatch(events,'install');assert.deepEqual(calls,['cached','skip']);await dispatch(events,'activate');assert.deepEqual(calls,['cached','skip','mainichi-dinner-v5','claim']);});
